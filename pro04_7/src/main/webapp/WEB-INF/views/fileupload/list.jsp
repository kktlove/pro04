    ...
    <div class="contents">
        <nav aria-label="You are here:" role="navigation">
            <ul class="breadcrumbs">
                ...
                <li>
                    <span class="show-for-sr">Current: </span> FileBoard List
                </li>
            </ul>
        </nav>
        <form action="${path1}/fileboard/list2.do" method="post">
        ...
        <table class="inner-table">
            ...
            <tbody>
                <c:forEach var="board" items="${boardList}">
                    <tr>
                        <td>${board.num}</td>
                        <td><a href="${path1}/fileboard/get.do?num=${board.num}">${board.title}</a></td>
                        <td>${board.regDate}</td>
                    </tr>
                </c:forEach>
            ...
            <ul class="pagination" style="width:230px;margin:10px auto;">
                <c:forEach var="i" begin="1" end="${page.pageCount}" step="1">
                    ...
                    <c:if test="${page.curpage!=i and (empty query)}">
                        <li><a href="${path1}/fileboard/list.do?curpage=${i}">${i}</a></li>
                    </c:if>
                    <c:if test="${page.curpage!=i and !(empty query)}">
                        <li><a href="${path1}/fileboard/list2.do?curpage=${i}&field=${field}&query=${query}">${i}</a></li>
                    </c:if>
        ...
        <ul class="button-group" style="list-style:none;width:100px;margin:10px auto;">
            <c:if test="${not empty sid}">
                <li><a href="${path1 }/fileboard/write.do" class="button">글 추가</a></li>
            </c:if>
        </ul>
    </div>
    <!-- 푸터 부분 인클루드 -->
